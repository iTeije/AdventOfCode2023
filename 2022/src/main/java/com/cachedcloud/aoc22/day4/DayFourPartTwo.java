...
public class DayFourPartTwo {
    ...
    public static void main(String[] args) {
        ...
        for (String line : input) {
            ...
            if (firstRange.start >= secondRange.start && firstRange.start <= secondRange.start) {
                count++;
            } else if (firstRange.end >= secondRange.start && firstRange.end <= secondRange.end) {
                count++;
            } else if (secondRange.end >= firstRange.start && secondRange.end <= firstRange.end) count++;
        }

        System.out.println("Result (2022 D4P2): " + count);
    }
