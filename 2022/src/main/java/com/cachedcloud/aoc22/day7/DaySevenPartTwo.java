...
public class DaySevenPartTwo {
    ...
    public static void main(String[] unusedArgs) {
        ...
        long rootSize = getDirSize(main, count);

        long minDirSize = rootSize + 30_000_000 - 70_000_000;

        AtomicLong smallest = new AtomicLong(70_000_000);
        getSmallestDirectory(main, minDirSize, smallest);

        System.out.println("Result (2022 D7P2): " + smallest.get());
    }
    ...
    public static long getSmallestDirectory(AOCDirectory directory, long minSize, AtomicLong currentMin) {
        long dirSize = 0;
        // Loop through child directories
        for (AOCDirectory childDir : directory.directories.values()) {
            long childDirSize = getSmallestDirectory(childDir, minSize, currentMin);
            if (childDirSize >= minSize && childDirSize < currentMin.get()) currentMin.set(childDirSize);
        ...
        if (dirSize >= minSize && dirSize < currentMin.get()) currentMin.set(dirSize);
        return dirSize;
    }
